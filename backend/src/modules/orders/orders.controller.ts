...
export async function placeOrder(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!shippingAddressId || !paymentMethod) {
      return sendError(res, [{ message: 'shippingAddressId and paymentMethod are required' }], 'Validation failed', 400);
    }
    ...
    if (!cart || cart.items.length === 0) {
      return sendError(res, [{ message: 'Your cart is empty' }], 'Cart is empty', 400);
    }
    ...
    if (!address || address.userId !== userId || address.isDeleted) {
      return sendError(res, [{ message: 'Invalid shipping address' }], 'Invalid address', 400);
    }
    ...
    let discount = 0;
    if (couponCode) {
      coupon = await prisma.coupon.findUnique({ where: { code: couponCode, isDeleted: false } });
      if (!coupon) {
        return sendError(res, [{ message: 'Invalid coupon code' }], 'Invalid coupon', 400);
      }
      if (coupon.expiresAt && new Date(coupon.expiresAt) < new Date()) {
        return sendError(res, [{ message: 'Coupon has expired' }], 'Coupon expired', 400);
      }
      if (coupon.usageLimit && coupon.usedCount >= coupon.usageLimit) {
        return sendError(res, [{ message: 'Coupon usage limit reached' }], 'Coupon fully used', 400);
      }
      if (coupon.minOrderValue && subtotal < Number(coupon.minOrderValue)) {
        return sendError(res, [{ message: `Minimum order value for this coupon is $${coupon.minOrderValue}` }], 'Minimum amount not met', 400);
      }
    ...
    return sendSuccess(res, order, 'Order placed successfully', 211);
  } catch (error: any) {
...
export async function getOrderById(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  try {
    ...
    if (!order || order.isDeleted) {
      return sendError(res, [{ message: 'Order not found' }], 'Not found', 404);
    }

    // Auth check: Admin can view any order, User can only view their own
    if (order.userId !== userId && !roles.includes('Admin')) {
      return sendError(res, [{ message: 'Unauthorized access' }], 'Forbidden', 403);
    }
...
export async function updateOrderStatus(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    if (!status || !validStatuses.includes(status)) {
      return sendError(res, [{ message: `Status must be one of: ${validStatuses.join(', ')}` }], 'Invalid status', 400);
    }
