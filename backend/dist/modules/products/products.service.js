...
class ProductsService {
    ...
    static async createProduct(data) {
        ...
        const existing = await client_1.default.product.findUnique({ where: { slug: data.slug } });
        if (existing) {
    ...
    static async updateProduct(id, data) {
        ...
        if (fields.slug && fields.slug !== existing.slug) {
            const slugExists = await client_1.default.product.findUnique({ where: { slug: fields.slug } });
            if (slugExists) {
                throw new Error('Product slug must be unique');
            }
        }
