...
function getClient() {
    if (!config_1.default.resendApiKey || config_1.default.resendApiKey.startsWith('re_REPLACE')) {
        console.warn('[EmailService] RESEND_API_KEY not configured — emails will be skipped.');
        return null;
    }
    return new resend_1.Resend(config_1.default.resendApiKey);
}
// ─── Email Templates ────────────────────────────────────────────────────────
...
function loginAlertTemplate(firstName, loginTime, email) {
    return baseTemplate(`
    <h1 style="margin:0 0 8px;font-size:22px;font-weight:700;color:#1a1a1a;letter-spacing:-0.02em;">
...
class EmailService {
     ...
     */
    static async sendLoginAlert(to, firstName) {
        const client = getClient();
        if (!client)
            return;
        try {
